//variables para establecer conexion con la api
    ...
    xhttp.onReadyStateChange = function (event) {
        ...
        if (xhttp.readyState === 4) {
            ...
            if (xhttp.status === 200) {
                ...
                //El atributo devuelve un DOMString que contiene la respuesta a la consulta
                //como un texto o null si la consulta no tuvo exito o aun no ha sido completada.
                callback(null, JSON.parse(xhttp.responseText))
                //si la respuesta de la API no es exitosa se captura el error
            } else {
                //se inicializa un objeto de tipo Error donde se le envian como argumentos un mensaje
                //de error y la URL de la API para conocer en dónde se produjo el error
...
//se invoca el metodo llamadoAPI() pasandole como argumentos la varible API concatenada
//con la cadena 'products' para acceder a la URL de la API deseada, y una función anónima
//que recibe 2 parámetros (un objeto de error y un arreglo que almacena todos los objetos traidos por la API).
llamadoAPI(`${endPoindAPI}/products`, function (error1, data1) {
    ...
    //se invoca nuevamente la función llamadoAPI con el fin de acceder a un objeto puntual
    //del arreglo data1, se envia como parámetros la url de la API apuntando al atributo 
    //del primer objeto de arreglo data1 y nuevamente una función anónima.
    llamadoAPI(`${endPoindAPI}/products/${data1[0].id}`, function (error2, data2) {
        ...
        //Se invoca nuevamente la funcion llamadoAPI con el fin de acceder a la categoria, se envían como parametros la url de la API con la concatenación de 'Categories' y el atributo Id de categoria del objeto data2 de la función anterior
